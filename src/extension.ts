...
function loadConfig(): { workDir: string, confPath: string, configs: any } {
	const workspaceFolder = (vscode.workspace.workspaceFolders || [])[0];
	if (!workspaceFolder) {
		vscode.window.showErrorMessage('Please open a workspace first.');
		return { workDir: '', confPath: '', configs: null };
	}
	const configPath = `${workspaceFolder.uri.fsPath}/.vscode/kube-debug.json`;
	const configContent = fs.readFileSync(configPath, 'utf-8');
	const config = JSON.parse(configContent);
	const renderedConfig = resolveVariables(config);
	return { workDir: renderedConfig.cwd || workspaceFolder.uri.fsPath, confPath: configPath, configs: renderedConfig };
}
